...
class SuguruLevelsGUI:
    def __init__(self, root, initial_puzzles, initial_size_label="8x8", initial_path=None):
        ...
        self.height = 8
        self.layout = None
        self.engine: Optional[LevelEngineRegions] = None
        self.board = None
    ...
    def _format_cell_coords(self, cells: List[int]) -> str:
        return ", ".join(f"({r+1},{c+1})" for r, c in (i2rc(idx, self.width) for idx in cells))
    ...
    def on_select(self, event=None):
        ...
        self.engine = LevelEngineRegions(self.width, self.height, self.layout, self.current["givens"])
    ...
    def color_guess_region(self, region_label: Optional[str], level_k: int, assignment: List[int]):
        if region_label is None:
            return []
        cells = self.regions.get(region_label, [])
        affected = []
        for idx, val in zip(cells, assignment):
            if self.givens_mask[idx]:
                continue
            self.draw_value(idx, val, tentative=False, color_override="#0d6efd")
            self.set_badge_level(idx, level_k)
            affected.append(idx)
        self.root.update()
        self.redraw_pencilmarks()
        for idx in affected:
            self.flash_cell(idx, "#dde7ff", ms=120)
        return affected

    def highlight_region(self, region_label: Optional[str], cells: Optional[List[int]], cand_count: Optional[int]=None):
        if not region_label or not cells:
            return
        for idx in cells:
            self.flash_cell(idx, color="#fff3b0", ms=160)
        coords = self._format_cell_coords(cells)
        msg = f"MRV região {region_label} ({coords})"
        if cand_count is not None:
            msg += f" — {cand_count} permutações"
        self.log(msg, "mrv")
    ...
    def process_events_log(self, events: List[dict]):
        for ev in events or []:
            ...
            if t == "region_mrv":
                ...
                cells = ev.get("cells", [])
                cand_count = ev.get("candidate_count")
                self.highlight_region(region, cells, cand_count)
            elif t == "contradiction_region":
                region = ev.get("region")
                assignment = ev.get("assignment")
                reason = ev.get("reason","")
                coords = self._format_cell_coords(self.regions.get(region, [])) if region else "?"
                self.log(f"Contradição na região {region} [{coords}] (perm={assignment}) — {reason}", "contradiction")
            elif t == "rollback_region":
                rev = ev.get("reverted", [])
                region = ev.get("region")
                self.log(f"Rollback região {region}: revertidas {len(rev)} casas", "rollback")
                if region and region in self.regions:
                    for idx in self.regions[region]:
                        self.set_badge_level(idx, None)
            elif t == "det_fills":
                cnt = ev.get("count",0)
                self.log(f"Deduções determinísticas: +{cnt}", "det")
            elif t == "commit_region":
                region = ev.get("region")
                coords = self._format_cell_coords(self.regions.get(region, [])) if region else "?"
                self.log(f"Commit região {region} ({coords})", "commit")
            elif t == "no_region_candidate":
                regs = ev.get("regions", [])
                self.log(f"Regiões sem candidatos: {', '.join(regs)}", "contradiction")
            elif t == "unsat_state":
                self.log("Estado inconsistente: sem regiões selecionáveis.", "contradiction")
            elif t == "solved":
    ...
    def run_one_level(self):
        ...
        if status == "level_committed":
            ...
            affected = self.color_guess_region(region, level_k, assignment)
            coords = self._format_cell_coords(self.regions.get(region, [])) if region else "?"
            if region:
                self.log(f"Nível {level_k} fixado na região {region} ({coords})", "commit")
            if affected:
                self.root.after(self.delay_ms)
            extra = f"Nível {level_k} fixado p/ região {region} ({i}/{j})."
            if info.get("fully"):
    ...
    def _autorun_tick(self):
        ...
        if status == "level_committed":
            ...
            level_k = info["level"]
            i,j = info["brother_pos"]
            self.color_guess_region(region, level_k, assignment)
            coords = self._format_cell_coords(self.regions.get(region, [])) if region else "?"
            extra = f"Auto: nível {level_k} fixado na região {region} ({i}/{j})."
            if info.get("fully"):
                extra += " (completo)"
            self.update_status(extra)
        elif status == "solved":
